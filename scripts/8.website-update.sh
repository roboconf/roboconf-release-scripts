...
echo
echo "Verifying pcregrep and PERL are installed..."
echo

# We use "sed" for most of the replacements.
# But there are sections with multi-line replacements.
# And "sed" is so bad for these use cases we need other tools.

pcregrep --version
ensureSuccess $? "pcregrep is not installed"

perl -v
ensureSuccess $? "PERL is not installed"
...
echo
echo "Downloading swagger.json files..."
echo

cd "${DIR}/resources/swagger.json"

curl http://repo1.maven.org/maven2/net/roboconf/roboconf-swagger/${RELEASE_VERSION}/roboconf-swagger-${RELEASE_VERSION}-swagger-rest-api.json -O -J -s
curl http://repo1.maven.org/maven2/net/roboconf/roboconf-swagger/${RELEASE_VERSION}/roboconf-swagger-${RELEASE_VERSION}-swagger-web-socket.json -O -J -s



echo
echo "Upgrading the links related to Swagger..."
echo

DIR="$(localStagingDirectory ${ROBOCONF_WEBSITE})"
cd "${DIR}/en/developer-guide"

# Extract the template from the page
section=$(pcregrep -M '<!--START(.|\n)*END-->' rest-api.md)

# Remove our custom mark-ups
...
# Now, update the version in it
section=${section//%v_SNAP%/"${RELEASE_VERSION}"}

# Eventually, insert it into the page
escaped_section=${section//\//\\/}
perl -pi -e "s/<!-- RELEASE_MARKER -->/<!-- RELEASE_MARKER -->\n\t<!-- Version ${RELEASE_VERSION} -->\n${escaped_section}/g" rest-api.md



echo
echo "Upgrading the guide for programmer APIs..."
echo
...
# Eventually, insert it into the page
escaped_section=${section//\//\\/}
perl -pi -e "s/<!-- RELEASE_MARKER -->/<!-- RELEASE_MARKER -->\n\n${escaped_section}/g" programmers-api.md
